...
スマホ/iPad/PCは外部マイクから音声入力を取得する。外部機材がない場合は、PC内蔵マイクを精度限定の簡易モードとして利用できる。

要件：

- 外部マイクを認識できる
- 内蔵マイクと外部マイクを区別できる
- 入力レベルを相対値（0〜100）とdBFS参考値で表示できる
- 選択したマイクをテーブル表示の騒音メーターでも引き継げる
- 接続切断を検知できる
- 外部マイク未接続時は、精度低下を明示して内蔵マイク簡易モードへ進める

内蔵マイク簡易モードの数値は、物理的な騒音dBではない。PCの機種、設置位置、OSの自動ゲインに依存するため、同じPC・同じ配置における相対変化の目安として扱う。
...
Android携帯をテーブル中央寄りに置き、以下を単一画面で利用できる。

- 携帯内蔵マイクによるRMS/ピーク計測
- 店内音量カテゴリ、0〜100相対値、dBFS参考値
- テーブル向けの丁重アラート表示
- 下部パネルから9種の幹事通知を送信
- モードB/Cでの手動画者時間記録
- 終了レポートへの導線
- Screen Wake Lockによる表示維持（対応ブラウザのみ）
- ユーザー操作による表示専用／全画面モード
- 表示専用モードで5秒無操作時にヘッダーと操作パネルを自動非表示
- 画面タップによる操作UI再表示と、明示的な終了ボタン

PWA要件：

- Web App Manifestを提供する
- 192px/512pxおよびmaskableアイコンを提供する
- Service Workerを登録し、静的UIをキャッシュする
- `display: standalone`、縦向きを標準とする
- HTTPS配信またはlocalhostでインストール・マイク利用を行う
- API通信はService Workerでキャッシュしない

PWAの `standalone` 表示ではブラウザのアドレスバーとタブを隠す。Fullscreen APIはブラウザの安全制約により必ずユーザー操作を起点とし、自動的・強制的に全画面化しない。Androidのシステムナビゲーションバーは端末設定によって残る場合がある。

携帯1台モード開始時は、開始前宣言と同意を省略しない。携帯内蔵マイクの数値は絶対騒音dBではなく、同一端末・同一配置での相対値として表示する。
...
外部マイク、またはPC内蔵マイク簡易モードの入力から以下を計算する。
...
dBFSは端末内のデジタル入力レベルであり、校正済み騒音計のdB SPLではない。内蔵マイク利用時は、カテゴリ判定と0〜100の相対値も参考表示として扱う。
